...
{
    public partial class FormPolygonShape2 : Form
    {
        /// How to use
        ...
        private PointF ptCheck = new(834594, 821023);//IN
        ...
        public FormPolygonShape2()
        {
            InitializeComponent();
        }

        //private static bool IsPointInsideTrapezoid(float minX, float minY, float maxX, float maxY, PointF pointToTest)
        //{
        ...
        private void FormPolygonShape2_MouseClick(object sender, MouseEventArgs e)
        {
            ptCheck = new PointF(e.X / _zoom + _shiftX, e.Y / _zoom + _shiftY);
            this.Invalidate();
        }

        private void FormPolygonShape2_MouseMove(object sender, MouseEventArgs e)
        {
            /// get current MousPos relative to the minima of the trapez and zoom
            PointF ptCur = new(e.X / _zoom + _shiftX, e.Y / _zoom + _shiftY);
            this.Text = this.Text.Split(new string[] { " ### " }, StringSplitOptions.RemoveEmptyEntries)[0] + " ### " + ptCur.ToString();
        }

        private void FormPolygonShape2_Paint(object sender, PaintEventArgs e)
        {
            ...
            using (System.Drawing.Drawing2D.GraphicsPath myPath = new())
            {
                myPath.AddLines(pt);
                myPath.CloseFigure();

                using (Pen pen = new(Color.Blue, 4))
